...
if ($demo_id > 0) {
  $sql = "select * from demo1 where demo_id = '$demo_id'";
  // echo "select * from demo where demo_id = '$demo_id'"; die;
  // print_r($sql);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  // print_r($_SERVER['REQUEST_METHOD'] == 'POST');die;
  $todo_id = $_POST['todo_id'];
  $status = $_POST['status'];
  $mobile = $_POST['mobile'];

  if ($demo_id == 0) {
    $sql = "insert into demo1 set
            todo_id = '$todo_id',status = '$status',mobile = '$mobile'";
    $res = mysqli_query($conn, $sql);
  } else {
    $sql = "update demo1 set
        todo_id = '$todo_id', status = '$status', mobile = '$mobile'
          ...
          <div class="mb-3">
            <div class="row g-3 align-items-center">
              <div class="col-lg-12 col-sm-12">
                ...
                <div>
                  <label class="fw-bolder" for="mobile">Todo Mobile</label>
                  <input type="text" class="form-control" id="mobile" name="mobile" value="<?php echo $mobile; ?>">
                  <b>
                    <div id="mobileError" class="formerror"></div>
                  </b>
                </div><br />
              </div>
      ...
      <tbody>
        ...
        while ($row = mysqli_fetch_assoc($result)) {
          ...
          echo " <tr>
                    ...
                    <td>" . $row['status'] . "</td>
                    <td>" . $row['mobile'] . "</td>
                    <td><a href='demo1.php?demo_id=$row[demo_id]' class='btn btn-sm btn-primary'>Edit</a></td>
...
<script type="text/javascript">
  function getData(todo_id) {
    ...
    $.ajax({
      type: 'POST',
      url: 'ajax_fetch_status1.php', // PHP script to fetch status and mobile based on todo_id
      data: {
        todo_id: todo_id
      },
