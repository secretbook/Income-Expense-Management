...
$tablename = "demo";
$tablekey = "demo_id";
$pagename = "geturl_combo_combo.php";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if ($demo_id == 0) {
    $sql = "insert into demo set
            todo_id = '$todo_id',task_id = '$task_id'";
    $res = mysqli_query($conn, $sql);
  } else {
    $sql = "update demo set
        todo_id = '$todo_id', task_id = '$task_id'
          ...
          <div class="mb-3">
            <div class="row g-3 align-items-center">
              <div class="col-lg-12 col-sm-12">
                ...
                <!-- <div class="col-lg-12 col-sm-12"> -->
                <div>
                  <label class="fw-bolder" for="textarea">Task Description</label>
                  <select class="form-select" name="task_id" id="task_id" aria-label="Default select example">
                    <option value="" selected>--Task Description--</option>
                    <?php
                    // Fetch data from the 'todo' table
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                      // Generate an option for each row in the 'todo' table
                      // Check if this option matches the todo_id of the record being edited
                      $selected = ($row['task_id'] == $task_id) ? "selected" : "";
                    }
                    ?>
                  </select>
                  <b>
                    <div id="task_idError" class="formerror"></div>
                  </b>
                </div><br />
    ...
    <table class="table table-striped">
      ...
      <tbody>
        ...
        while ($row = mysqli_fetch_assoc($result)) {
          ...
          $task_sql = "SELECT task_desc FROM `task` WHERE task_id = '" . $row['task_id'] . "'";
          ...
          echo " <tr>
                    ...
                    <td>" . $task_row['task_desc'] . "</td>
                    <td><a href='geturl_combo_combo.php?demo_id=$row[demo_id]&task_id=$row[task_id]' class='btn btn-sm btn-primary'>Edit</a></td>
...
function getData(todo_id) {
  // Make an AJAX request to fetch task_id based on the selected todo_id
  $.ajax({
    type: 'POST',
    url: 'ajax_fetch_combo_combo.php', // PHP script to fetch task_id based on todo_id
    data: {
      todo_id: todo_id
    },
    success: function(response) {
      // Update the task_id select field with the fetched data
      $("#task_id").html(response).trigger('chosen:updated');
      // $('#task_id').html(response);
    }
  });

 // For edit mode, if task_id is already set, select it in the dropdown
 var editTaskId = <?php echo json_encode($task_id); ?>;
  if (editTaskId !== '') {
    $('#task_id').val(editTaskId).trigger('chosen:updated');
  }
}
</script>
<!-- <script type="text/javascript">
  function getData(todo_id) {
    ...
    $.ajax({
      type: 'POST',
      url: 'ajax_fetch_combo_combo.php', // PHP script to fetch status based on todo_id
      data: {
        todo_id: todo_id
      },
      success: function(response) {
